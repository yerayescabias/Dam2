...
$(document).ready(function () {
    ...
    $(".Trash").click(function (e) {
        ...
        var recordToEliminate = $(this).attr("data-id"); 
        var guztira = parseFloat($("#cart-total").text().replace(/[^\d.-]/g, '')); 

        if (recordToEliminate !== '') {
            $.ajax({
                url: "/Saskia/SaskitikBorratuAjax", 
                type: "POST",
                data: {
                    id: recordToEliminate, 
                    guztira: guztira       
                },
                success: function (data) {
                    if (data.kendu) { 
                        
                        $("#row-" + recordToEliminate).remove();
                        ...
                        $("#update-message").html(`<div class="alert alert-danger">NErrorea.</div>`);
                    }
                },
                error: function (xhr, status, error) {
                    console.error("Error removing from cart:", error);
